...
/**
 * [游戏王制卡器](https://ymssx.gitee.io/ygo/)
 */
public sealed class YgoCard {
    ...
    /**
     * @param mold [https://gitee.com/ymssx/cardjs/tree/master/source/mold]
     */
    public fun render(mold: File = File(System.getProperty("xyz.cssxsh.skia.ygo", "."))): Surface {
        mold.resolve("font").walk().forEach { file ->
            try {
        ...
        // draw frame
        val frame = when (this) {
            is Spell -> mold.resolve("frame/spell.jpg")
            is Trap -> mold.resolve("frame/trap.jpg")
            is Monster -> mold.resolve("frame/monster_xg.jpg")
        }
        ...
        // draw attribute
        val attribute = Image.makeFromEncoded(mold.resolve("attribute/${locale.country}/${attribute}.png").readBytes())
        canvas.drawImageRect(attribute, Rect.makeXYWH(680F, 57F, 75F, 75F))
        ...
        when (this) {
            is Monster -> {
                val star = Image.makeFromEncoded(mold.resolve("star/level.png").readBytes())
                repeat(level) { index ->
